...
if(loginForm){
  loginForm.addEventListener('submit', (e)=>{
    ...
    if(email==='admin@fleximovers.nz' && pass==='admin123'){
      localStorage.setItem('fm_admin_logged','true');
      showApp();
    } else {
      const err = document.getElementById('loginError');
      if(err) err.textContent='Invalid - use admin@fleximovers.nz / admin123';
    }
  });
}
...
window.logout = function(){
  localStorage.removeItem('fm_admin_logged');
  location.reload();
}
